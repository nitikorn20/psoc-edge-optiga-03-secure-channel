 ...
 * Platforms can define both MAILBOX_IS_UNCACHED_S and MAILBOX_IS_UNCACHED_NS
 * to shrink the mailbox struct a bit if it cannot be cached.
 */
#if defined(MAILBOX_IS_UNCACHED_S) && defined(MAILBOX_IS_UNCACHED_NS)
...
struct mailbox_slot_t {
    /* In the array, msg must not share a cache line with the preceeding reply */
