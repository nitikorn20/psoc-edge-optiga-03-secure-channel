                                    ...
                                    struct mailbox_reply_t *reply)
{
    ...
    result = mtb_srf_ipc_pool_req_alloc(cybsp_mtb_srf_client_context.pool, &request, MTB_SRF_IPC_SERVICE_TIMEOUT_US);
    if (result != CY_RSLT_SUCCESS) {
        return MAILBOX_GENERIC_ERROR;
    }

    /* Allocate semaphore */
    result = mtb_srf_ipc_pool_semaphore_alloc(cybsp_mtb_srf_client_context.pool, &sema_idx, MTB_SRF_IPC_SERVICE_TIMEOUT_US);
    if (result != CY_RSLT_SUCCESS) {
        return MAILBOX_GENERIC_ERROR;
    }
    ...
    result = mtb_srf_ipc_request_send(&cybsp_mtb_srf_client_context, request, MTB_IPC_NEVER_TIMEOUT);
    ...
    sema_dealloc_rslt = mtb_srf_ipc_pool_semaphore_free(cybsp_mtb_srf_client_context.pool, sema_idx);

    /* Deallocate request */
    req_dealloc_rslt = mtb_srf_ipc_pool_req_free(cybsp_mtb_srf_client_context.pool, request);

    if ((result != CY_RSLT_SUCCESS) || (sema_dealloc_rslt != CY_RSLT_SUCCESS) || (req_dealloc_rslt != CY_RSLT_SUCCESS)) {
        return MAILBOX_GENERIC_ERROR;
    }
