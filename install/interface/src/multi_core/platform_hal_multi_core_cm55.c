...
#define IFX_IS_REGION_IN_ITCM_MEMORY(base, size)   \
            (ifx_is_region_inside_other(base, base + size, \
                                        ...
                                        (CY_CM55_ITCM_INTERNAL_NS_SBUS_BASE + CY_CM55_ITCM_INTERNAL_SIZE)) == true)
...
{
    return IFX_IS_REGION_IN_ITCM_MEMORY_REMAPPED(base, size) || IFX_IS_REGION_IN_DTCM_MEMORY_REMAPPED(base, size);
}
...
void* tfm_hal_remap_ns_cpu_address(const void* addr)
{
    ...
    if (IFX_IS_REGION_IN_ITCM_MEMORY((uintptr_t)addr, 0U)) {
        return (void*)(((uint32_t)addr)- CY_CM55_ITCM_INTERNAL_NS_SBUS_BASE + CY_CM55_ITCM_NS_SBUS_BASE);
    } else if (IFX_IS_REGION_IN_DTCM_MEMORY((uintptr_t)addr, 0U)) {
        return (void*)(((uint32_t)addr)- CY_CM55_DTCM_INTERNAL_NS_SBUS_BASE + CY_CM55_DTCM_NS_SBUS_BASE);
    } else {
